    ...
    function connect() {
        ...
        const handleEvent = (event, type) => {
            try {
                ...
                if (type === 'log' && data.result.indexOf('执行步骤') > -1) {
            ...
            fetch(`/tasks/${taskId}`)
                ...
                .then(task => {
                    if (task.status === 'completed' || task.status === 'failed') {
                        ...
                        retryCount++;
                        showErrorToast(`连接断开，${retryDelay / 1000}秒后重试（${retryCount}/${maxRetries}）`)
                        setTimeout(connect, retryDelay);
                    } else {
                        showErrorToast('连接断开，请尝试刷新页面')
                    }
                })
                .catch(error => {
...
function applyHistory(tasks) {
    ...
    if (tasks.length === 0) {
        historyList.innerHTML = '<li class="list-group-item text-muted">未找到记录！</li>';
    } else {
        ...
        tasks.forEach(item => {
            ...
            li.addEventListener('click', function () {
                ...
                item.steps.forEach(step => {
                    ...
                    if (step.type === 'log' && step.result.indexOf('执行步骤') > -1) {
...
function addMessage(text, sender) {
    ...
    if (sender === 'user') {
        ...
        <button class="copy-btn" onclick="copyMessage(this)" title="复制">
