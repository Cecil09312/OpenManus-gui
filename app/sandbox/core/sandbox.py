    ...
    async def run_command(self, cmd: str, timeout: Optional[int] = None) -> str:
        ...
        try:
            ...
            raise SandboxTimeoutError(
                f"命令执行在 {timeout or self.config.timeout} 秒后超时"
            )
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                with tarfile.open(tar_path) as tar:
                    ...
                    if not members:
                        raise FileNotFoundError(f"源文件为空：{src_path}")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                try:
                    ...
                    raise RuntimeError(f"验证文件创建失败：{dst_path}")
    ...
    async def _read_from_tar(tar_stream) -> bytes:
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            with tarfile.open(fileobj=tmp) as tar:
                member = tar.next()
                if not member:
                    raise RuntimeError("空的 tar 归档")

                file_content = tar.extractfile(member)
                if not file_content:
                    raise RuntimeError("提取文件内容失败")
        ...
        try:
            ...
            if self.container:
                try:
                    ...
                    errors.append(f"容器停止错误：{e}")

                try:
                    await asyncio.to_thread(self.container.remove, force=True)
                except Exception as e:
                    errors.append(f"容器删除错误：{e}")
                finally:
                    self.container = None

        except Exception as e:
            errors.append(f"一般清理错误：{e}")

        if errors:
            print(f"警告：清理过程中出现错误：{', '.join(errors)}")
