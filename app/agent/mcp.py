...
class MCPAgent(ToolCallAgent):
    """用于与 MCP（模型上下文协议）服务器交互的智能体。

    此智能体通过 SSE 或 stdio 传输连接到 MCP 服务器，
    并通过智能体的工具接口使服务器的工具可用。
    """

    name: str = "mcp_agent"
    description: str = "一个连接到 MCP 服务器并使用其工具的智能体。"
    ...
    async def initialize(
        ...
        if self.connection_type == "sse":
            if not server_url:
                raise ValueError("SSE 连接需要服务器 URL")
            await self.mcp_clients.connect_sse(server_url=server_url)
        elif self.connection_type == "stdio":
            if not command:
                raise ValueError("stdio 连接需要命令")
    ...
    async def _refresh_tools(self) -> Tuple[List[str], List[str]]:
        ...
        if added_tools:
            ...
            self.memory.add_message(
                Message.system_message(f"新增可用工具：{', '.join(added_tools)}")
            )
        if removed_tools:
            logger.info(f"已移除 MCP 工具：{removed_tools}")
            self.memory.add_message(
                Message.system_message(
        ...
        if changed_tools:
            logger.info(f"已变更 MCP 工具：{changed_tools}")
