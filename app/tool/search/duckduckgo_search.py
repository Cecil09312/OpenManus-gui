    ...
    def perform_search(
        ...
        for i, item in enumerate(raw_results):
            if isinstance(item, str):
                ...
                results.append(
                    SearchItem(
                        title=f"DuckDuckGo 结果 {i + 1}", url=item, description=None
                    )
                )
                ...
                results.append(
                    SearchItem(
                        title=item.get("title", f"DuckDuckGo 结果 {i + 1}"),
                        url=item.get("href", ""),
                ...
                try:
                    results.append(
                        SearchItem(
                            title=getattr(item, "title", f"DuckDuckGo 结果 {i + 1}"),
