...
class BingSearchEngine(WebSearchEngine):
    ...
    def _search_sync(self, query: str, num_results: int = 10) -> List[SearchItem]:
        """
        同步 Bing 搜索实现，用于获取搜索结果。

        参数：
            query (str): 提交给 Bing 的搜索查询。
            num_results (int, 可选): 返回结果的最大数量。默认为 10。

        返回：
            List[SearchItem]: 包含标题、URL 和描述的搜索项列表。
        """
        if not query:
            return []
    ...
    def _parse_html(
        ...
        """
        解析 Bing 搜索结果 HTML，提取搜索结果和下一页 URL。
        ...
        try:
            ...
            logger.warning(f"解析 HTML 时出错：{e}")
            return [], None
