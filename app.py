...
async def run_task(task_id: str, prompt: str):
    try:
        ...
        agent = Manus(
            name="Manus",
            description="一个能够使用多种工具解决各种任务的通用智能体",
        )
        ...
        async def on_tool_execute(tool, input):
            await task_manager.update_task_step(
                task_id, 0, f"执行工具：{tool}\n输入：{input}", "tool"
            )

        async def on_action(action):
            await task_manager.update_task_step(
                task_id, 0, f"执行操作：{action}", "act"
            )
        ...
        class SSELogHandler:
            ...
            async def __call__(self, message):
                ...
                event_type = "log"
                if "✨ Manus 的思考：" in cleaned_message:
                    event_type = "think"
                elif "🛠 Manus 选择了" in cleaned_message:
                    event_type = "tool"
                elif "🎯 工具" in cleaned_message:
                    event_type = "act"
                elif (
                    "📝 出错了！" in cleaned_message or "🚨 出错了！" in cleaned_message
                ):
                    event_type = "error"
                elif "🏁 特殊工具" in cleaned_message:
                    event_type = "complete"
    ...
    async def event_generator():
        ...
        while True:
            try:
                ...
                print(f"任务 {task_id} 的客户端已断开连接")
                break
            except Exception as e:
                print(f"事件流中出错：{str(e)}")
...
async def generic_exception_handler(request: Request, exc: Exception):
    return JSONResponse(status_code=500, content={"message": f"服务器错误：{str(exc)}"})
...
def load_config():
    try:
        ...
        print(f"配置文件缺少必要字段：{str(e)}，使用默认配置")
        return {"host": "localhost", "port": 5172}
